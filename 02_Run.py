...
M=1     #Mass
E_w=0    #Coefficient of Restitution between Wedge and Ball
E_b=0    #Coefficient of Restitution between Balls
F_w=0  #Coefficient of Friction between Wedge and Ball
F_b=0    #Coefficient of Friction between Balls

#Simulation Parameters
dt=1/100            #Time Interval at which positions are updated
dv=0.1              #Maximum Velocity for stability
...
Text=False          #Prints status(Turning it off with Animation=False might test your patience)
...
for iteration in range(1,Runs+1):
    ...
    balls = []
    Running = True
    my_seed = r.randrange(sys.maxsize)
    r.seed(my_seed)
    ...
    def shady_stuff():
        ...
        if Dynamic: 
            ...
            Al=-A/2
            Ar=A/2
        file = Data(*encrypt(N=N, R=R, A=A,
                            Fix=Fix, Stable=Stable, Infinite=Infinite, Rotate=Rotate, Local=Local,Dynamic=Dynamic,
                            Gaussian=Gaussian,Hypothetic=Hypothetic,H=H, T=T, W=W, g=g, M=M, Ew=E_w, Eb=E_b, Fw=F_w, Fb=F_b))
    ...
    def touche():
        ...
        if Stable:
            ...
            if sf: Running = False
        elif all([balls[i].isStable() for i in range(n.val)]):
            Running = False
        if not ib:
            regret()
    ...
    def local(vel=None, change=None, dyn=None, **kwargs):
        global left, right, Al, Ar
    ...
    if Local:
        S.on_collision(1, 2,
                       begin=partial(sim.handle, event=partial(local, vel=True, change=True,dyn=False)))
        if Dynamic:
            S.on_collision(1, 0,
                            begin=partial(sim.handle, event=partial(local, vel=False, change=True,dyn=True)))
        else:
            S.on_collision(1, 0,
                           begin=partial(sim.handle, event=partial(local, vel=False, change=True, dyn=False)))
        S.on_collision(2, 2,
                       begin=partial(sim.handle, event=partial(local, vel=True, change=False,dyn=False)))
        S.on_collision(2, 0,  # Dropped Ball and Wedge
                       begin=partial(sim.handle, event=partial(local, vel=True, change=False,dyn=False)))

    elif Dynamic:
        S.on_collision(1, 0,  # Ball and Wedge
                       begin=partial(sim.handle, event=partial(local, vel=False, change=False,dyn=True)))
